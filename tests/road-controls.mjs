...
await p.mouse.move(120, 500);
await p.keyboard.down("w");
await p.waitForFunction(() => window.__batwing.state.groundPosition[2] < 425);
...
await p.mouse.move(920, 500);
await p.keyboard.down("w");
await p.waitForFunction(() => window.__batwing.ground.car.yaw < -0.1);
await p.mouse.up({ button: "right" });
await p.keyboard.up("w");
assert.equal(await p.evaluate(() => window.__batwing.ground.controls().steer), 0);
const result = await p.evaluate(() => {
  const g = window.__batwing.ground;
  g.car.position.set(13, 0.6, 300);
  g.car.yaw = -Math.PI / 2;
  g.car.speed = 70;
  ...
  for (let i = 0; i < 300; i++) {
    g.update(1 / 60);
    max = Math.max(max, g.car.position.x);
  }
  g.keys.KeyW = false;
  g.keys.ShiftLeft = false;
  return { max, contact: g.car.roadContact, position: g.car.position.toArray() };
});
assert.ok(result.max <= 13.5 + 0.001, JSON.stringify(result));
...
console.log(
  "PASS: idle cursor straight driving, deliberate mouse steering/release, boosted curb collision, reset, no errors",
  result,
);
await browser.close();
