...
try {
  for (const mobile of [false, true]) {
    ...
    await p.waitForFunction(() => {
      const c = document.getElementById("radio-caption");
      return c && !c.hidden;
    });
    ...
    await p.evaluate(() => {
      ...
      g.audio.stopVoice();
      g.car.position.set(-210, 0.6, 0);
      g.car.yaw = Math.PI / 2;
      g.car.checkpoint = 1;
      g.cameraRig.reset(g.car.position, g.car.yaw);
      g.radio("ALFRED / Keep to the lit corridor.");
      g.update(0.016);
    });
    ...
    await p.evaluate(() => {
      ...
      g.phase = "play";
      g.start();
      g.car.position.set(-1100, 0.6, -890);
      g.car.yaw = Math.PI / 2;
      g.car.checkpoint = 5;
      g.cameraRig.reset(g.car.position, g.car.yaw);
      g.camera.position.copy(g.cameraRig.eye);
      g.look.copy(g.cameraRig.look);
      g.update(0.016);
      g.update(3);
    });
    assert.ok(
      await p.evaluate(
        () => window.__batwing.ground.routeScenes.shelterGlass.emissiveIntensity > 1,
      ),
    );
    assert.ok(await p.evaluate(() => window.__batwing.ground.camera.fov < 54));
    await p.waitForTimeout(1000);
    await p.screenshot({
      path: `verification/premium-arrival-${mobile ? "mobile" : "desktop"}.png`,
    });
    await p.click("#arrival-skip");
    await p.click("#restart");
    assert.equal(
      await p.evaluate(() => window.__batwing.ground.routeScenes.shelterGlass.emissiveIntensity),
      0.08,
    );
  ...
  console.log(
    "PASS desktop/mobile single radio, arrival restoration, replay reset and reduced motion",
  );
} finally {
  await browser.close();
}
