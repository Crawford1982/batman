...
const result = await p.evaluate(() => {
  ...
  return {
    hit,
    hp: enemy.hp,
    label: document.querySelector("#target-label").textContent,
...
assert.match(result.label, /BOMBER/);
assert.ok(result.width > 0 && result.width < 100);
await p.screenshot({ path: "verification/target-feedback.png" });
await p.evaluate(() => {
  const g = window.__batwing;
  g.start();
  for (const e of g.enemies) e.mesh.position.set(5000, 100, 5000);
  g.flight.yaw = Math.PI;
  g.step(0.01);
});
...
await p.evaluate(() => {
  ...
  g.car.position.set(0, 0.6, 100);
  g.car.speed = 0;
  for (let i = 0; i < 100; i++) g.update(0.05, 0);
  g.phase = "paused";
});
assert.deepEqual(
  await p.evaluate(() => window.__batwing.ground.junctionGuide.markers.map((m) => m.visible)),
  [true, false, false, false, false, false],
);
await p.screenshot({ path: "verification/junction-guide.png" });
await p.evaluate(() => {
  const g = window.__batwing.ground;
  g.junctionGuide.update(1, { x: 0, z: 20 });
});
assert.ok(await p.evaluate(() => window.__batwing.ground.junctionGuide.markers[0].visible));
assert.deepEqual(errors, []);
await b.close();
console.log(
  "PASS: actual cannon hit feedback and armour, target loss, nearby junction arrows retained after crossing, no browser errors",
);
