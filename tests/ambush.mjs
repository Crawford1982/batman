...
try {
  for (const mobile of [false, true]) {
    ...
    await p.evaluate(() => {
      ...
      g.car.checkpoint = 2;
      g.car.position.set(-522.5, 0.6, -480);
      g.update(0.016);
      g.camera.position.copy(g.car.position).add({ x: 0, y: 4, z: 14 });
    });
    assert.equal(await p.evaluate(() => window.__batwing.ground.ambushState), "active");
    assert.match(await p.locator("#drive-ambush").innerText(), /IN EMP RANGE/);
    ...
    if (mobile) await p.locator("#drive-touch-emp").tap();
    else await p.keyboard.press("f");
    ...
    await p.evaluate(() => window.__batwing.ground.update(0.1));
    assert.equal(await p.evaluate(() => window.__batwing.ground.car.score), 500);
    await p.evaluate(() => {
      const g = window.__batwing.ground;
      g.start();
    });
    assert.equal(await p.evaluate(() => window.__batwing.ground.ambushState), "waiting");
    assert.equal(
      await p.evaluate(() => window.__batwing.ground.ambushMines.some((m) => m.visible)),
      false,
    );
    await p.evaluate(() => {
      const g = window.__batwing.ground;
      g.car.checkpoint = 2;
      g.car.position.set(-510, 0.6, -400);
      g.update(0.016);
      g.car.position.z = -640;
      g.update(0.016);
    });
  ...
  console.log(
    "PASS ambush activation, desktop/mobile EMP, one-time bonus, clean bypass and replay reset",
  );
} finally {
  await b.close();
}
