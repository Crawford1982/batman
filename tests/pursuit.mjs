...
try {
  for (const mobile of [false, true]) {
    ...
    const warning = await page.evaluate(() => {
      ...
      g.car.checkpoint = 1;
      g.car.position.set(-180, 0.6, 0);
      g.car.yaw = Math.PI / 2;
      g.cameraRig.reset(g.car.position, g.car.yaw);
      g.updatePursuit(0.01);
      return { phase: g.pursuitState, strike: g.strikeTime };
    });
    ...
    assert.equal(await page.evaluate(() => window.__batwing.ground.pursuitTime), timer);
    ...
    const outcomes = await page.evaluate(() => {
      ...
      g.updateStrike(3);
      g.updatePursuit(0.01);
      const hit = g.car.score;
      setup();
      g.resetRoad();
      g.updatePursuit(3);
      const reset = g.car.score;
      setup();
      g.car.position.x = -400;
      g.updatePursuit(0.01);
      const outrun = g.car.score;
      g.start();
      return { evade, once, hit, reset, outrun, replay: g.pursuitState };
    });
  ...
  console.log(
    "PASS pursuit warning, pause, desktop/mobile EMP, evasion, damage, outrun, recovery and replay",
  );
} finally {
  await browser.close();
}
