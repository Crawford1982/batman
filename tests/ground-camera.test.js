...
test("chase camera takes the shortest path across yaw wrap", () => {
  const rig = new GroundCamera(),
    p = new Vector3();
  rig.reset(p, Math.PI - 0.01);
  rig.update(1 / 60, { position: p, yaw: -Math.PI + 0.01, steer: 0, speed: 20 }, false);
  assert.ok(Math.abs(rig.yaw - Math.PI) < 0.02);
  assert.ok(rig.eye.z < 0);
});
test("camera dampens steering aim and settles consistently at 30 and 60 fps", () => {
  const run = (fps) => {
    const r = new GroundCamera(),
      p = new Vector3();
    r.reset(p);
    for (let i = 0; i < fps * 2; i++)
      r.update(1 / fps, { position: p, yaw: 0.5, steer: 1, speed: 40 }, true);
    return r;
  };
  assert.ok(run(30).eye.distanceTo(run(60).eye) < 0.02);
  assert.ok(run(30).look.distanceTo(run(60).look) < 0.02);
  const r = new GroundCamera(),
    p = new Vector3();
  r.reset(p);
  const before = r.look.clone();
