...
for (const mobile of [false, true]) {
  ...
  assert.ok(
    await p.evaluate(() => window.__batwing.ground.audio.master.gain.value < 0.13),
    "music ducks under dialogue",
  );
  const durations = await p.evaluate(async () =>
    (await window.__batwing.ground.audio.preloadVoices()).map((buffer) => buffer?.duration),
  );
  ...
  assert.ok(
    await p.evaluate(() => window.__batwing.ground.audio.master.gain.value > 0.23),
    "music returns after skipping",
  );
  const cancelled = await p.evaluate(async () => {
    const audio = window.__batwing.ground.audio;
    const original = audio.loadVoice.bind(audio);
    let resolve;
    audio.loadVoice = () => new Promise((r) => (resolve = r));
    const pending = audio.speak("alfred");
    audio.stopVoice();
    resolve(await original("alfred"));
    await pending;
    audio.loadVoice = original;
  ...
  await p.evaluate(async () => {
    ...
    await a.speak("alfred");
    a.mute(true);
    await a.speak("gordon");
  });
...
await b.close();
console.log(
  "PASS: all MP3s decode, three voices follow captions, skip/mute cancel speech, pending download cancellation, desktop/mobile, no page errors",
);
