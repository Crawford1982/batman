...
for (const mobile of [false, true]) {
  ...
  await page.screenshot({
    path: `verification/attack-warning-${mobile ? "mobile" : "desktop"}.png`,
  });
  const state = await page.evaluate(() => {
    const g = window.__batwing,
      e = g.enemies[0];
    for (let i = 0; i < 200 && e.ai.phase === "warning"; i++) g.step(0.01);
...
await browser.close();
console.log(
  "PASS: visible desktop/mobile attack warning, warned shot, escort assignment without extra fighters, bomber loss detachment, no page errors",
);
