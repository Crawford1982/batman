...
try {
  ...
  assert.ok(await p.locator("#chapter-results").isHidden());
  await p.keyboard.press("Escape");
  assert.ok(await p.locator("#chapter-card").isHidden());
  await p.setViewportSize({ width: 390, height: 844 });
  await p.waitForFunction(
    () => document.querySelector("#game").getBoundingClientRect().width <= 390,
  );
  await p.evaluate(() => {
    window.__batwing.start();
    window.__batwing.finish(false);
  ...
  await p.evaluate(async () => {
    localStorage.setItem(
      "gotham-best-results",
      JSON.stringify({ batmobile: { time: 240, score: 100 } }),
    );
