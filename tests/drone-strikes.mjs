...
try {
  for (const mobile of [false, true]) {
    ...
    const opening = await page.evaluate(() => {
      ...
      const warned =
        g.openingWarned &&
        g.strikeTime === 0 &&
        document.getElementById("drive-radio").textContent.includes("They've found you");
      for (let i = 0; i < 31; i++) g.update(0.1, 0.1);
      ...
      g.start();
      return { quiet, warned, strike, checkpoint };
    });
    ...
    await page.evaluate(() => {
      const g = window.__batwing.ground;
      g.car.checkpoint = 2;
      g.car.position.set(-522.5, 0.6, -700);
      g.car.speed = 0;
      g.ambushState = "complete";
      g.strikeNumber = 1;
      g.beginStrike();
      g.updateStrike(0);
    ...
    assert.equal(
      await page.evaluate(
        () => window.__batwing.ground.strikeMarkers.filter((m) => m.visible).length,
      ),
      3,
    );
    ...
    if (mobile) await page.locator("#drive-touch-emp").tap();
    else await page.keyboard.press("f");
    assert.equal(await page.evaluate(() => window.__batwing.ground.car.score), 150);
    assert.equal(
      await page.evaluate(() => window.__batwing.ground.strikeMarkers.some((m) => m.visible)),
      false,
    );
    ...
    const outcomes = await page.evaluate(() => {
      ...
      g.car.position.z -= 50;
      g.updateStrike(4);
      const evade = g.car.score;
      g.updateStrike(4);
      const once = g.car.score;
      g.car.invulnerable = 0;
      g.beginStrike();
      g.updateStrike(4);
      const hit = g.car.health;
      g.car.position.set(-522.5, 0.6, -950);
      g.beginStrike();
      const corner = g.strikeTime;
      g.start();
      g.car.damage(6);
      g.car.position.set(0, 0.6, 20);
      g.update(0.016);
  ...
  console.log(
    "PASS desktop/mobile barrage, EMP cooldown, evasion, single damage, corner safety, section bonuses and replay",
  );
} finally {
  await browser.close();
}
