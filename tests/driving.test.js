...
test("car steering, boost cap and wall impact", () => {
  const c = new Driving();
  for (let i = 0; i < 600; i++) c.update(1 / 60, { accel: 1, steer: 1, boost: true });
  assert.ok(c.yaw < 0);
...
test("road junctions remain passable and off-road shortcuts are rejected", () => {
  const c = new Driving();
  c.position.set(0, 0.6, 8);
  c.yaw = Math.PI / 2;
  for (let i = 0; i < 360; i++) c.update(1 / 60, { accel: 1 });
  assert.ok(c.position.x < -100);
  assert.ok(carFitsRoad(c.position, c.yaw));
...
test("idle cursor and controller drift do not pull the car left", () => {
  const mouse = { active: true, x: -0.9 };
  assert.equal(driveSteering({}, mouse, {}, null), 0);
  assert.equal(driveSteering({}, mouse, {}, { axes: [-0.12] }), 0);
  assert.equal(driveSteering({ KeyD: true }, mouse, {}, { axes: [-0.8] }), 1);
  assert.ok(driveSteering({}, { ...mouse, steering: true }, {}, null) < 0);
  const c = new Driving();
  for (let i = 0; i < 600; i++)
    c.update(1 / 60, { accel: 1, steer: driveSteering({}, mouse, {}, null) });
  assert.equal(c.position.x, 0);
...
test("low frame rates preserve mission time and stable car motion", () => {
  ...
  for (let i = 0; i < 20; i++) b.update(0.1, { accel: 1 });
  assert.ok(Math.abs(a.elapsed - b.elapsed) < 1e-8);
  assert.ok(Math.abs(a.position.z - b.position.z) < 0.1);
  const before = b.elapsed;
  b.update(0.05, {}, [], true, 1.5);
...
test("road recovery preserves distance, mission time and earned progress", () => {
  const c = new Driving();
  c.checkpoint = 2;
  c.position.set(-510, 0.6, -700);
  c.elapsed = 110;
  c.health = 60;
  c.score = 1000;
  c.recoverToRoute();
  assert.equal(c.position.x, -522.5);
  assert.equal(c.position.z, -700);
  assert.ok(carFitsRoad(c.position, c.yaw));
  assert.equal(c.elapsed, 110);
  assert.equal(c.health, 60);
  assert.equal(c.checkpoint, 2);
