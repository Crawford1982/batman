...
try {
  ...
  assert.ok(await p.locator("#pause-menu").isVisible());
  assert.ok(await p.locator("#arrival-film").isHidden());
  assert.deepEqual(errors, []);
  console.log("PASS arrival framing, frozen timer, automatic results and no runtime errors");
} finally {
  await b.close();
}
