...
const checked = await p.evaluate(async () => {
  const T = await import("/node_modules/three/build/three.module.js"),
    g = window.__batwing.ground;
  g.audio.stopVoice();
  const analyser = g.audio.ctx.createAnalyser();
  analyser.fftSize = 2048;
  g.audio.driveScore.connect(analyser);
  ...
  g.audio.driveScore.disconnect(analyser);
  analyser.disconnect();
  g.phase = "paused";
  g.exhaust.visible = true;
  ...
  g.camera.lookAt(g.car.position.clone().add(new T.Vector3(0, 0.3, 3)));
...
assert.equal(checked.root, 0);
assert.equal(checked.parent, true);
assert.equal(checked.height, 0.65);
await p.screenshot({ path: "verification/exhaust-after.png" });
console.log(
  "PASS real driving music has midrange output; exhaust anchored to body at zero-based geometry root",
  checked,
);
await b.close();
