...
export class FeedbackQueue {
  ...
  hit(amount, now) {
    this.hitAt = now;
    this.amount = Math.min(1, Math.max(0, amount) / 20);
...
export class PlayerFeedback {
  constructor(audio) {
    ...
    this.edge.id = "damage-feedback";
    this.edge.setAttribute("aria-hidden", "true");
    document.body.append(this.edge);
  ...
  reset() {
    this.queue.reset();
    this.edge.classList.remove("active");
  ...
  reward(text, position) {
    this.queue.reward(text, position.clone(), performance.now());
  }
  update(camera, active) {
    const now = performance.now(),
      age = now - this.queue.hitAt;
    this.edge.classList.toggle("active", active && age < 150);
    if (active && age < 300 && !this.motion.matches) {
      const amplitude = this.queue.amount * 0.22 * (1 - age / 300);
      camera.position.x += Math.sin(age * 0.09) * amplitude;
      camera.position.y += Math.cos(age * 0.11) * amplitude * 0.5;
      camera.updateMatrixWorld();
    }
    this.queue.items = this.queue.items.filter((item) => now - item.at < 1200);
    this.nodes.forEach((node, i) => {
      const item = this.queue.items[i];
      node.hidden = !active || !item;
      if (node.hidden) return;
      this.point.copy(item.position).project(camera);
      if (this.point.z < -1 || this.point.z > 1) {
        node.hidden = true;
        return;
      }
