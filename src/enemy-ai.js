...
export function updateEnemy(enemy, player, dt) {
  ...
  const ai = (enemy.ai ||= {
    phase: "approach",
    clock: 0,
    cooldown: 6 + (enemy.phase || 0),
    aim: new Vector3(),
    direction: new Vector3(),
    side: (enemy.phase || 0) > 3 ? 1 : -1,
  });
  ai.clock += dt;
  ...
  if (ai.phase === "approach") {
    ...
    if (p.distanceTo(player.position) < 420 && ai.cooldown <= 0) {
      ...
      const lead =
        ATTACK_WARNING +
        Math.max(0, p.distanceTo(player.position) - (25 + player.speed) * ATTACK_WARNING) /
          (95 + player.speed);
    ...
    enemy.mesh.lookAt(p.clone().add(ai.direction));
    enemy.mesh.rotateZ(ai.side * 0.4);
    if (ai.clock >= 3.5) {
      ai.phase = "approach";
      ai.clock = 0;
      ai.cooldown = 4;
