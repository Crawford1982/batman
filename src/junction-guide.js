...
export class JunctionGuide {
  constructor(parent) {
    ...
    shape.lineTo(1.1, 0);
    shape.closePath();
    const base = new T.ShapeGeometry(shape);
    ...
    this.markers = DRIVE_ROUTE.map((goal, i) => {
      const a = ROAD_POINTS[i],
        b = DRIVE_ROUTE[i + 1] || goal;
      const incoming = new T.Vector3(goal.x - a.x, 0, goal.z - a.z).normalize();
      const yaw = Math.atan2(-incoming.x, -incoming.z);
      const outgoing =
        i + 1 < DRIVE_ROUTE.length ? Math.atan2(-(b.x - goal.x), -(b.z - goal.z)) : yaw;
      const turn = Math.atan2(Math.sin(outgoing - yaw), Math.cos(outgoing - yaw));
      const pieces = [65, 40, 15].map((distance, n) => {
        const geo = base.clone();
        geo.rotateY(yaw + (turn * n) / 2);
        geo.translate(goal.x - incoming.x * distance, 0.25, goal.z - incoming.z * distance);
        return geo;
      });
      const mesh = new T.Mesh(mergeGeometries(pieces), mat);
  ...
  update(checkpoint, position) {
    this.markers.forEach((mesh, i) => {
      ...
      mesh.visible =
        (i === checkpoint && distance < 230) || (i === checkpoint - 1 && distance < 75);
    });
  }
}
