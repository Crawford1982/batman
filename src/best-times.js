...
export function recordBest(storage, chapter, seconds, score, win) {
  ...
  const completed =
    win && Number.isFinite(seconds) && seconds >= 0 && Number.isFinite(score) && score >= 0;
  const improvement =
    completed && previous && seconds < previous.time ? previous.time - seconds : null;
  const best = completed
    ? {
        time: Math.min(previous?.time ?? Infinity, seconds),
