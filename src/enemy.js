...
const hullParts = [],
  lightParts = [];
function part(list, geometry, position, rotation = [0, 0, 0], scale = [1, 1, 1]) {
...
part(hullParts, new T.CylinderGeometry(0.7, 1.5, 8, 6), [0, 0, 0], [Math.PI / 2, 0, 0]);
part(hullParts, new T.ConeGeometry(1.25, 4, 6), [0, 0, 5], [Math.PI / 2, 0, 0]);
for (const side of [-1, 1]) {
  part(hullParts, new T.BoxGeometry(7, 0.45, 3), [side * 4, -0.25, -1.2], [0, side * 0.38, 0]);
  part(
    hullParts,
  ...
  part(hullParts, new T.BoxGeometry(0.35, 2.6, 2.4), [side * 2, 1, -3.5], [0, 0, -side * 0.35]);
  part(
    lightParts,
...
part(lightParts, new T.SphereGeometry(1, 8, 6), [0, 0.85, 1.7], [0, 0, 0], [0.65, 0.4, 1.65]);
...
for (const side of [-1, 1]) {
  part(bomberParts, new T.BoxGeometry(10, 0.65, 5), [side * 5.3, -0.25, -1], [0, side * 0.32, 0]);
  part(
    bomberParts,
    new T.CylinderGeometry(1.25, 1.25, 7, 8),
    [side * 5, -0.3, -2],
    [Math.PI / 2, 0, 0],
  );
  part(bomberParts, new T.BoxGeometry(0.45, 3.5, 3), [side * 3, 1.6, -4.5], [0, 0, -side * 0.22]);
  part(
    bomberParts,
    new T.CylinderGeometry(0.65, 0.65, 5, 6),
    [side * 2, -1.3, 1],
    [Math.PI / 2, 0, 0],
  );
  part(
    bomberLights,
    new T.CylinderGeometry(0.88, 0.88, 0.25, 8),
    [side * 5, -0.3, -5.6],
    [Math.PI / 2, 0, 0],
  );
  part(bomberLights, new T.BoxGeometry(2, 0.2, 0.5), [side * 9, -0.15, -3]);
}
part(bomberLights, new T.BoxGeometry(2, 0.5, 1.8), [0, 1.15, 3]);
const bomberHull = mergeGeometries(bomberParts),
  bomberGlow = mergeGeometries(bomberLights);
const bomberArmor = new T.MeshStandardMaterial({
  color: 0x647380,
  metalness: 0.65,
  roughness: 0.4,
});
