...
export class GroundCamera {
  constructor() {
    ...
    this.targetEye = new Vector3();
    this.targetLook = new Vector3();
  ...
  update(dt, car, boosting, reducedMotion = false) {
    ...
    this.yaw +=
      Math.atan2(Math.sin(car.yaw - this.yaw), Math.cos(car.yaw - this.yaw)) *
      (1 - Math.exp(-dt * 10));
    ...
    this.targetEye
      .set(-this.steer * 0.65, 3.4 + speed * 0.2, 13 + this.boost * 1.5)
      .applyAxisAngle(this.up, this.yaw)
      .add(car.position);
    this.targetLook
      .set(-this.steer * 1.5, 1.3, -17 - speed * 4)
      .applyAxisAngle(this.up, car.yaw)
      .add(car.position);
