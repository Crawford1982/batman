...
export class ChapterHandover {
  constructor({ ground, camera, player, flight, world, audio, prepare, complete, exit }) {
    ...
    this.copy = this.panel.querySelector("p");
    ...
    document.getElementById("handover-exit").onclick = () => {
      this.cancel();
      exit();
    };
    this.token = 0;
  ...
  start() {
    ...
    this.title.textContent = "THE SKIES ARE CLEAR.";
  ...
  async load() {
    ...
    this.copy.textContent = this.revealed
      ? "Preparing the Batmobile…"
      : "The override is secured. Gotham still needs you.";
    try {
      await this.ground.load();
      if (token !== this.token || !this.active) return;
      this.copy.textContent = "Deliver the override to Gordon at the cathedral.";
    } catch {
      if (token !== this.token || !this.active) return;
      this.failed = true;
      this.copy.textContent = "Vehicle download interrupted. Retry when you’re ready.";
      document.getElementById("handover-retry").hidden = false;
    }
  }
  reveal() {
    ...
    this.copy.textContent = this.failed
      ? "Vehicle download interrupted. Retry when you’re ready."
      : this.ground.ready
        ? "Deliver the override to Gordon at the cathedral."
        : "Preparing the Batmobile…";
  ...
  cancel() {
    ...
    document.body.classList.remove("handover-active");
  }
  update(dt) {
    ...
    if (!this.revealed) {
      ...
      this.camera.lookAt(p.clone().add(new T.Vector3(0, 1, 0)));
      this.camera.fov = 44;
