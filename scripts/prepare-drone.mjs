...
for (const m of j.materials) {
  ...
  if (old) {
    ...
    if (old.diffuseTexture) m.pbrMetallicRoughness.baseColorTexture = old.diffuseTexture;
    delete m.extensions.KHR_materials_pbrSpecularGlossiness;
  }
}
j.extensionsUsed = (j.extensionsUsed || []).filter(
  (e) => e !== "KHR_materials_pbrSpecularGlossiness",
);
j.extensionsRequired = (j.extensionsRequired || []).filter(
  (e) => e !== "KHR_materials_pbrSpecularGlossiness",
);
...
const io = new NodeIO(),
  doc = await io.readBinary(Buffer.concat([header, json, tail]));
await MeshoptSimplifier.ready;
// Redundant UV channels on this export unnecessarily restrict simplification.
for (const m of doc.getRoot().listMeshes())
  for (const p of m.listPrimitives())
    for (const a of ["TEXCOORD_1", "TEXCOORD_2", "TEXCOORD_3"]) p.setAttribute(a, null);
await doc.transform(
  dedup(),
  weld(),
