...
for (const name of ["Building_Medium_2_001", "Building_Small_1"]) {
  ...
  for (const mesh of d.getRoot().listMeshes())
    for (const p of mesh.listPrimitives())
      if (/MI_Interior/.test(p.getMaterial()?.getName() || "")) p.dispose();
  await d.transform(
    prune(),
    weld(),
  ...
  d.getRoot().listScenes()[0].setName(name);
  mergeDocuments(combined, d);
}
...
const scenes = combined.getRoot().listScenes(),
  scene = combined.createScene("TheatreTemplates");
for (const s of scenes) {
  const root = combined.createNode(s.getName());
  for (const n of s.listChildren()) root.addChild(n);
  scene.addChild(root);
  s.dispose();
}
...
console.log({
  ...
  triangles: root
    .listMeshes()
    .reduce(
      (n, m) => n + m.listPrimitives().reduce((s, p) => s + p.getIndices().getCount() / 3, 0),
      0,
    ),
});
